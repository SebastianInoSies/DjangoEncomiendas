...
class ClienteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cliente
        fields = ['id', 'tipo_doc', 'nro_doc', 'nombres', 'apellidos',
...
class HistorialEstadoSerializer(serializers.ModelSerializer):
    ...
    estado_nuevo_bg = serializers.ReadOnlyField()

    class Meta:
        model = HistorialEstado
        fields = ['id', 'estado_anterior', 'estado_anterior_display',
                  'estado_nuevo', 'estado_nuevo_display', 'estado_nuevo_bg',
...
class EncomiendaSerializer(serializers.ModelSerializer):
    ...
    estado_bg = serializers.ReadOnlyField()

    class Meta:
        model = Encomienda
        fields = ['id', 'codigo', 'descripcion', 'descripcion_corta',
                  'peso_kg', 'volumen_cm3', 'costo_envio',
                  'remitente', 'destinatario', 'ruta', 'empleado_registro',
                  'estado', 'estado_display', 'estado_bg',
    ...
    def validate(self, data):
        ...
        if ruta and costo is not None and float(costo) < float(ruta.precio_base):
...
class EncomiendaListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Encomienda
        fields = ['id', 'codigo', 'estado', 'estado_display', 'estado_bg',
...
class EncomiendaDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Encomienda
        fields = ['id', 'codigo', 'descripcion', 'peso_kg',
                  'remitente', 'remitente_id',
                  'destinatario', 'destinatario_id',
                  'ruta', 'ruta_id',
                  'estado', 'estado_bg', 'costo_envio',
                  'fecha_registro', 'fecha_entrega_est', 'fecha_entrega_real',
                  'esta_entregada', 'tiene_retraso', 'dias_en_transito',
...
class EncomiendaV2Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'codigo', 'descripcion', 'descripcion_corta',
                  ...
                  'destinatario', 'destinatario_id',
                  'ruta', 'ruta_id',
                  'estado', 'estado_bg', 'fecha_registro', 'fecha_entrega_est',
