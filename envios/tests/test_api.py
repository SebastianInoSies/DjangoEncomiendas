...
class TestListadoEncomiendas:
    ...
    def test_lista_respuesta_paginada(self):
        EncomiendaFactory(remitente=self.cliente1, destinatario=self.cliente2,
    ...
    def test_filtro_por_estado(self):
        enc_pe = EncomiendaFactory(estado='PE', ruta=self.ruta,
            remitente=self.cliente1, destinatario=self.cliente2, empleado_registro=self.empleado)
...
class TestCrearEncomienda:
    ...
    def test_crear_exitoso_devuelve_201(self):
        response = self.client.post(reverse('encomienda-list', kwargs={'version': 'v1'}), self.data_valida, format='json')
    ...
    def test_crear_asigna_empleado_del_token(self):
        self.client.post(reverse('encomienda-list', kwargs={'version': 'v1'}), self.data_valida, format='json')
    ...
    def test_sin_auth_no_crea_y_devuelve_401(self, api_client):
        response = api_client.post(reverse('encomienda-list', kwargs={'version': 'v1'}), self.data_valida, format='json')
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
class TestCambiarEstado:
    def setup_method(self):
        self.user = UserFactory()
        self.empleado = EmpleadoFactory(email=self.user.email)
        self.enc = EncomiendaFactory(empleado_registro=self.empleado, estado='PE')
        self.client = APIClient()
        refresh = RefreshToken.for_user(self.user)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {refresh.access_token}')
...
class TestAccionesPersonalizadas:
    def setup_method(self):
        ...
        self.enc_retraso = EncomiendaFactory(estado='TR', fecha_entrega_est=ayer,
            empleado_registro=self.empleado)
        self.enc_normal = EncomiendaFactory(estado='PE', empleado_registro=self.empleado)
